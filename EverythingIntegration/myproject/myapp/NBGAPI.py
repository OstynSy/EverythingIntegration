...
notion_client = Client(auth=None)


def oauth_redirect(request):
    # Construct the OAuth authorization URL
    auth_url = "https://api.notion.com/v1/oauth/authorize?client_id=14df4f07-340d-43ee-bce4-d01b02e8441f&response_type=code&owner=user&redirect_uri=https%3A%2F%2Fhttp%2F%2F127.0.0.1%3A8000%2F"
...
def notion_callback(request):
    ...
    # Create a Notion API client instance
    client = Client(auth=auth_code)

    # Retrieve the user's Notion database ID
    database_id = client.get_database_id()

    # Return a JSON response containing the auth code and database ID
    return JsonResponse({'auth_code': auth_code, 'database_id': database_id})
